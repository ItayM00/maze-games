...
public class FogMaze extends Maze{


    public FogMaze(int size, MazeGenerationStrategy generationStrategy, MazeSolvingStrategy solvingStrategy){
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < rowsNum; i++) {
            for (int j = 0; j < colsNum; j++) {
                ...
                if(cell.getIsDiscovred()) drawCellWalls(g2d, cell, x, y);
            }
        }
    }

    public void drawCellBackground(Graphics g, Cell cell, int x, int y) {;
        if(!cell.getHumanVisit() || !cell.getComputerVisit()){
